...
//    }
    ...
    public static void printDetails(Animal animal) { // polymorphic parameter
        animal.printDetails();
    }
    ...
    public static Animal createAnimal(String name, int age) {
        if (name.equals("Rex")) {
            ...
            animal.setName(name);
            return animal; // will return husky instance
        } else if (name.equals("Kitty")) {
            Animal cat = new Cat(age); // poly
            cat.setName(name);
            return cat;
        }

        Animal rabbit = new Rabbit(age); // Polymorphism
        rabbit.setName(name);
        return rabbit;
    }
}
